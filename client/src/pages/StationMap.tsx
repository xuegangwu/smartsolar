  ...
  useEffect(() => {
    ...
    import('leaflet').then(L => {
      ...
      filtered.forEach((s: Station) => {
        ...
        // Icon: white circle with colored border and emoji
        const icon = L.divIcon({
          html: `<div style="width:${size}px;height:${size}px;border-radius:50%;background:#fff;border:4px solid ${color};box-shadow:0 4px 16px ${color}50,0 0 0 ${sc === '#16a34a' ? '4px' : '0'} ${sc}30;display:flex;align-items:center;justify-content:center;font-size:${size * 0.45}px;cursor:pointer;transition:transform 0.15s"
