...
const TYPE_COLOR: Record<string, string> = {
  solar: '#d97706', storage: '#e6342a', solar_storage: '#16a34a',
};
...
export default function StationMap() {
    ...
    Promise.all([
      ...
      const mcg = L.markerClusterGroup({
        ...
        iconCreateFunction: (cluster: any) => {
          ...
          const stations = markers.map((m: any) => m.station as Station);
          // Color: if mixed types, use purple
          const types = [...new Set(stations.map((s: Station) => s.type))];
          const color = types.length > 1 ? '#7c3aed' : TYPE_COLOR[types[0]] || '#8896a6';
          const online = stations.filter((s: Station) => s.status === 'online').length;
          ...
          return L.divIcon({
            html: `
              ...
              ">
                ${count === 1
                  ? stations[0].type === 'solar_storage' ? '⚡' : stations[0].type === 'solar' ? '☀️' : '🔋'
                  : `<span style="font-size:${count > 9 ? 13 : 15}px;font-weight:800;color:${color};font-family:JetBrains Mono,monospace">${count}</span>`}
              </div>
              ${count > 1 ? `<div style="margin-top:2px;font-size:9px;color:${STATUS_COLOR[online === count ? 'online' : 'maintenance']};font-weight:700">${online}/${count}在线</div>` : ''}
            `,
            className: '',
            iconSize: L.point(size, count > 1 ? size + 16 : size),
  ...
  // Render/update markers with clustering
  useEffect(() => {
    ...
    import('leaflet').then(L => {
      ...
      filtered.forEach((s: Station) => {
        ...
        const statusColor = STATUS_COLOR[s.status] || '#b8c0cc';
        // Size based on capacity
        const cap = s.capacity || s.installedCapacity || s.peakPower || 0;
        const size = cap >= 1000 ? 44 : cap >= 500 ? 38 : cap > 0 ? 32 : 30;
        ...
        const icon = L.divIcon({
          html: `
            <div style="
              width:${size}px;height:${size}px;border-radius:50%;
              background:#fff;border:3px solid ${color};
              box-shadow:0 2px 10px ${color}40, 0 0 0 ${statusColor === '#16a34a' ? '3px' : '0'} ${statusColor}30;
              display:flex;align-items:center;justify-content:center;
        ...
        (marker as any).station = s; // attach for cluster iconCreateFunction
      ...
      // Re-add cluster click handler after clear
      mcg.on('clusterclick', (e: any) => {
        if (map.getZoom() < 12) e.layer.spiderfy();
      });
    });
  }, [filtered, loading]);
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 14 }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 6, flexWrap: 'wrap' }}>
            {[
              { label: '全部电站', value: stations.length, color: '#1a1a2e' },
              { label: '已标注', value: stations.filter(s => s.location?.lat && s.location?.lng && s.location.lat !== 0).length, color: '#16a34a' },
              { label: '在线', value: onlineCount, color: '#16a34a' },
              { label: '总装机', value: `${(totalCapacity / 1000).toFixed(1)}MW`, color: '#e6342a' },
            ].map(s => (
              <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <span style={{ fontSize: 12, color: '#8896a6' }}>{s.label}</span>
                <span style={{ fontSize: 14, fontWeight: 800, color: s.color, fontFamily: 'JetBrains Mono, monospace' }}>{s.value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Filter + tile controls */}
      <Card size="small" style={{ marginBottom: 12, border: '1px solid #e8eaed', borderRadius: 12 }}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center' }}>
          ...
          <Button.Group size="small">
            <Button icon={<GlobalOutlined />} onClick={() => setTileLayer('street')}
              style={{ fontSize: 11, height: 26, background: tileLayer === 'street' ? '#1a1a2e' : 'transparent',
                color: tileLayer === 'street' ? '#fff' : '#8896a6', borderColor: tileLayer === 'street' ? '#1a1a2e' : '#e8eaed' }}>
              街道
            </Button>
            <Button icon={<AppstoreOutlined />} onClick={() => setTileLayer('satellite')}
              style={{ fontSize: 11, height: 26, background: tileLayer === 'satellite' ? '#1a1a2e' : 'transparent',
                color: tileLayer === 'satellite' ? '#fff' : '#8896a6', borderColor: tileLayer === 'satellite' ? '#1a1a2e' : '#e8eaed' }}>
              卫星
            </Button>
          </Button.Group>
        </div>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div style={{
          ...
          {selected ? (
            <>
              <div style={{ padding: '16px 18px', borderBottom: '1px solid #e8eaed', background: '#fafafa', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                <div>
              ...
              <div style={{ flex: 1, overflow: 'auto', padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {[
                    { label: '装机容量', value: `${((selected.capacity || selected.installedCapacity || selected.peakPower || 0)).toFixed(0)}`, unit: 'kW', color: '#e6342a' },
                    ...
                    <div key={item.label} style={{ background: '#f8f9fa', borderRadius: 10, padding: '9px 11px' }}>
                      ...
                      <div style={{ fontSize: 13, fontWeight: 700, color: item.color, fontFamily: 'JetBrains Mono, monospace', lineHeight: 1.2, wordBreak: 'break-all' }}>{item.value}{item.unit && <span style={{ fontSize: 10, color: '#b8c0cc' }}> {item.unit}</span>}</div>
                    </div>
                  ))}
                </div>
                <div style={{ height: 1, background: '#f0f0f0' }} />
                ...
                {selected.createdAt && <Text style={{ fontSize: 10, color: '#b8c0cc', marginTop: 'auto' }}>创建于 {new Date(selected.createdAt).toLocaleDateString('zh-CN')}</Text>}
              </div>
            </>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 12 }}>
              <div style={{ fontSize: 36 }}>📍</div>
              <Text style={{ fontSize: 13, color: '#8896a6', textAlign: 'center', padding: '0 24px' }}>点击地图标记查看详情</Text>
              <div style={{ fontSize: 11, color: '#b8c0cc', textAlign: 'center' }}>
                <div>{stations.length} 座电站 · {stations.filter(s => s.location?.lat && s.location?.lng && s.location.lat !== 0).length} 座已标注</div>
                <div style={{ marginTop: 4 }}>总装机 {(totalCapacity / 1000).toFixed(1)} MW</div>
              </div>
            </div>
          )}
        </div>
        ...
        <Card title={<span style={{ fontSize: 13 }}>电站列表 <Badge count={filtered.length} style={{ backgroundColor: '#e6342a', fontSize: 10 }} /></span>}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(230px, 1fr))', gap: 8 }}>
            {filtered.map(s => {
              ...
              return (
                <div key={s._id} onClick={() => { setSelected(s); setPanelOpen(true); if (isGeo && mapInstRef.current) mapInstRef.current.flyTo([s.location.lat, s.location.lng], 12, { duration: 0.8 }); }}
                  style={{
