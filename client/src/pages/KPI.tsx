...
function KPICard({ title, value, suffix, sub, icon, color, tip }: {
  ...
  return (
    <Card
      size="small"
      style={{
        background: '#151d2e', border: `1px solid ${color}30`,
    ...
    >
      ...
      <div style={{ color: '#64748b', fontSize: 11, fontFamily: 'JetBrains Mono, monospace', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>
        {title}
      </div>
...
function DonutChart({ segments }: { segments: { label: string; value: number; color: string }[] }) {
  ...
  const strokes = segments.map((seg, i) => {
    ...
    const stroke = `<circle cx="40" cy="40" r="32" fill="transparent" stroke="${seg.color}" stroke-width="10" stroke-dasharray="${(pct * 201).toFixed(1)} 201" stroke-dashoffset="${(-offset * 201).toFixed(1)}" transform="rotate(-90 40 40)" style="transition: stroke-dasharray 1s" /><text x="40" y="36" text-anchor="middle" fill="${seg.color}" font-size="10" font-family="JetBrains Mono" font-weight="600">${seg.value}</text><text x="40" y="50" text-anchor="middle" fill="#64748b" font-size="8" font-family="JetBrains Mono">${seg.label}</text>`;
    offset += pct;
    return { pct, stroke };
  });
...
export default function KPI() {
  ...
  const availabilityData = [
    { label: '苏州工业园', value: 99.2, color: '#00D4AA' },
    { label: '无锡储能电站', value: 97.8, color: '#00b4d8' },
    { label: '杭州光储站', value: 98.6, color: '#a855f7' },
  ];

  const alertDistribution = [
    { label: '严重', value: 1, color: '#ef4444' },
    { label: '重要', value: 3, color: '#f59e0b' },
    { label: '一般', value: 12, color: '#64748b' },
  ];

  const woStatusData = [
    { label: '已关闭', value: 47, color: '#00D4AA' },
    { label: '处理中', value: 3, color: '#f59e0b' },
    { label: '待处理', value: 2, color: '#ef4444' },
  ];

  const priorityData = [
    { label: '紧急', value: 3, color: '#ef4444' },
    { label: '重要', value: 12, color: '#f59e0b' },
    { label: '一般', value: 37, color: '#00D4AA' },
  ];
  ...
  const columns: ColumnsType<any> = [
    { title: 'KPI 指标', dataIndex: 'name', render: v => <Text style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: '#94a3b8' }}>{v}</Text> },
    {
      ...
      render: v => <Text style={{ fontFamily: 'JetBrains Mono, monospace', fontWeight: 600, color: '#00D4AA' }}>{v}</Text>,
    },
    { title: '目标', dataIndex: 'target', render: v => <Text style={{ fontFamily: 'JetBrains Mono, monospace', color: '#64748b' }}>{v}</Text> },
    {
      title: '状态', dataIndex: 'status',
      render: s => (
        <Space>
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: s === 'green' ? '#00D4AA' : '#ef4444', boxShadow: `0 0 6px ${s === 'green' ? '#00D4AA' : '#ef4444'}` }} />
          <Text style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: s === 'green' ? '#00D4AA' : '#ef4444' }}>
            {s === 'green' ? '达标' : '未达标'}
          </Text>
        </Space>
      ),
  ...
  return (
    <div>
      ...
      <Row gutter={[12, 12]} style={{ marginBottom: 20 }}>
        <Col xs={12} sm={8}>
          <KPICard title="可用率 Availability" value={kpis.availability} suffix="%" icon={<CheckCircleOutlined />} color="#00D4AA" sub="MTBF: 8760h (全年无停机)" />
        </Col>
        <Col xs={12} sm={8}>
          <KPICard title="性能比 PR" value={kpis.pr} suffix="%" icon={<RiseOutlined />} color="#00b4d8" sub="目标 ≥85%" />
        </Col>
        <Col xs={12} sm={8}>
          <KPICard title="MTTR" value={kpis.mttr} suffix="h" icon={<ClockCircleOutlined />} color="#f59e0b" sub="目标 ≤4h" />
        </Col>
        <Col xs={12} sm={8}>
          <KPICard title="O&M 成本" value={kpis.omCostPerKwh} suffix="元/kWh" icon={<DollarOutlined />} color="#a855f7" sub="目标 ≤¥0.05" />
        </Col>
        <Col xs={12} sm={8}>
          <KPICard title="工单关闭率" value={woClosedPct} suffix="%" icon={<ToolOutlined />} color="#00D4AA" sub={`${kpis.workOrderClosed}/${kpis.workOrderTotal} 本月`} />
        </Col>
        <Col xs={12} sm={8}>
          <KPICard title="告警确认率" value={kpis.alertAckRate} suffix="%" icon={<ThunderboltOutlined />} color="#f59e0b" sub="目标 ≥90%" />
        </Col>
      </Row>
      ...
      <Row gutter={[12, 12]} style={{ marginBottom: 20 }}>
        ...
        <Col xs={24} lg={12}>
          <Card size="small" title={<Space><CheckCircleOutlined style={{ color: '#00D4AA' }} /><span>各电站可用率</span></Space>}>
        ...
        {/* Alert Distribution */}
        <Col xs={24} lg={12}>
          <Card size="small" title={<Space><ThunderboltOutlined style={{ color: '#f59e0b' }} /><span>告警级别分布</span></Space>}>
      ...
      <Row gutter={[12, 12]}>
        {/* Work Order Status */}
        <Col xs={24} lg={12}>
          <Card size="small" title={<Space><ToolOutlined style={{ color: '#00D4AA' }} /><span>本月工单状态</span></Space>}>
        ...
        {/* KPI Table */}
        <Col xs={24} lg={12}>
          <Card size="small" title={<Space><CheckCircleOutlined style={{ color: '#00D4AA' }} /><span>O&M KPI 达标概览</span></Space>}>
            <Table
              columns={columns}
